...
pub struct CertificateAuthority {
    ca_cert: Certificate,
    /// Cache des certificats par hostname
...
impl CertificateAuthority {
    /// Charge le CA depuis le disque ou en génère un nouveau
    pub async fn load_or_create(cert_dir: &str) -> Result<Self> {
        ...
        let ca_cert = if ca_cert_path.exists() && ca_key_path.exists() {
            ...
            tracing::info!(
                "⚠  Installez ce certificat comme CA racine de confiance sur vos appareils."
            );
            cert
        };
    ...
    async fn load_ca(cert_path: &PathBuf, key_path: &PathBuf) -> Result<Certificate> {
    ...
    /// Retourne (ou génère) un certificat signé par le CA pour l'hôte donné
    ...
    fn create_host_cert(&self, hostname: &str) -> Result<CachedCert> {
        ...
        // Wildcard si c'est un sous-domaine
        if let Some(domain) = hostname.split_once('.').map(|(_, d)| d) {
            if !domain.is_empty() && !domain.contains('.') == false {
...
fn pem_to_der_key(pem: &str) -> Result<Vec<u8>> {
    // Essayer PKCS8 d'abord, puis RSA
    ...
    let mut keys = rustls_pemfile::rsa_private_keys(&mut pem.as_bytes())
        .context("Parsing clé privée")?;
    keys.drain(..).next().context("Aucune clé privée trouvée")
}
