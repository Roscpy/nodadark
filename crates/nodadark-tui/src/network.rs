...
impl EngineClient {
    ...
    pub async fn connect_unix(path: &str) -> Result<Self> {
        ...
        Ok(Self { kind: ClientKind::Unix(conn), pending_events: VecDeque::new() })
    }
    ...
    pub async fn connect_tcp(addr: &str) -> Result<Self> {
        ...
        Ok(Self { kind: ClientKind::Tcp(conn), pending_events: VecDeque::new() })
    }

    pub fn demo_mode() -> Self {
        Self { kind: ClientKind::Demo, pending_events: VecDeque::new() }
    }

    /// Envoie une commande JSON au moteur
    ...
    fn process_line(&self, line: &str, app: &mut AppState) {
        // Essayer de parser comme ApiResponse
        if let Ok(resp) = serde_json::from_str::<serde_json::Value>(line) {
            match resp.get("type").and_then(|t| t.as_str()) {
                ...
                // Événements temps réel (type: request / response / dropped / ...)
                _ => {
